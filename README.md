...
1. Antes que nada, debemos correr desde consola `sudo service mongod start` para levantar el servidor de bases de datos. Una vez hecho esto, importar (o mejor dicho 'restaurar') la base de datos. Para esto, situados en la carpeta que contiene los archivos del repositorio debemos correr desde consola el comando `sudo mongorestore -d ProyectoLoopback ./DB`.
